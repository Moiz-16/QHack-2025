...
# Function needed from your original code
def solve_shortest_path_qaoa(graph, start_node, end_node):
    ...
    # Initial parameters (better than random - use a heuristic)
...
def repair_invalid_solution(graph, selected_edges, start_node, end_node):
    ...
    if not nx.has_path(path_graph, start_node, end_node):
        ...
        for node in path_graph.nodes():
            if nx.has_path(path_graph, node, end_node):
...
def visualize_heist_scenario(graph, bank_node, safehouse_node, qaoa_path=None, selected_edges=None, classical_path=None):
    ...
    # Use spring layout for city layout
    ...
    if qaoa_path and len(qaoa_path) > 1:
        ...
        legend_elements.append(mpatches.Patch(color='purple', label='QAOA Optimal Getaway Path'))

        # Draw the QAOA path intermediate locations with different color
    ...
    if classical_path and len(classical_path) > 1 and classical_path != qaoa_path:
        ...
        legend_elements.append(mpatches.Patch(color='blue', label='Classical Optimal Getaway Path'))
...
def main():
    ...
    try:
        ...
        num_nodes = 10  # Number of locations in the city
        max_edges = 18  # Maximum number of routes
